...
const AuthProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  useEffect(() => {
    ...
    if (isChromeExtension) {
      const handleMessage = (message: AuthMessage) => {
        if (!isMounted) return
  ...
  const signInWithGoogle = async () => {
    if (isChromeExtension) {
      try {
        ...
        if (!chrome.identity) {
          return {
            error: new Error('Chrome identity API를 사용할 수 없습니다.') as AuthError,
          }
        }
        ...
        return new Promise<{ error?: AuthError }>((resolve) => {
          ...
          const handleAuthMessage = (message: AuthMessage) => {
            if (authCompleted) return

            if (message.type === 'AUTH_STATE_CHANGED') {
              console.log('OAuth completed successfully via background message')
              authCompleted = true
          ...
          chrome.identity.launchWebAuthFlow(
            ...
            (resultUrl) => {
              ...
              if (chrome.runtime.lastError) {
                console.log('Chrome runtime error (expected):', chrome.runtime.lastError.message)
                // 에러가 있어도 바로 실패 처리하지 않고 background script 메시지를 기다림
                console.log('Ignoring Chrome runtime error, waiting for background message...')
              }

              // resultUrl이 undefined여도 oauth-callback.js가 실행되었을 수 있으므로 기다림
              if (!resultUrl) {
                console.log('No result URL, but waiting for background script message...')
              }
          ...
          setTimeout(() => {
            if (!authCompleted) {
              ...
              chrome.runtime.onMessage.removeListener(handleAuthMessage)
              resolve({
                error: new Error('OAuth 타임아웃 - 인증 메시지를 받지 못했습니다.') as AuthError,
              })
            }
          }, 45000)
        })
